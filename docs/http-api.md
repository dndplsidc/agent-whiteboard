...
The browser protocol uses WebSocket subprotocol `agent-whiteboard.v4`. Connect, submit, and New commands always carry a nullable `settings` field. Codex settings are the complete semantic tuple `{model, effort, speed}`; Pi uses `null`. Snapshots and settings events carry a strict visible catalog plus either verified effective settings or an explicit unverified state. Queue items carry their immutable captured settings, including broker-produced model presentation, and an accepted-turn settings event is the only browser-preference update authority.

Submit, queue, history, and user-message payloads carry `content.parts`: ordered text parts interleaved with revision-bound `text`, `section`, or `image` reference parts. Image commands and events contain only bounded opaque references/descriptors—never bytes or private filesystem paths. Older viewers and brokers are intentionally incompatible with this contract; deploy viewer and broker together and restart `agent serve` or its managed daemon before reconnecting.
