...
When Page Agent is enabled, each rendered Mermaid fence exposes **Add diagram**. The resulting ordered reference uses the existing Markdown `section` shape: its `markdown` field contains the exact fenced Mermaid source, `section_lines` contains the one-based end-exclusive source range, and its Markdown anchor is bound to the current resource revision and context digest. The reference does not contain rendered SVG or stage image bytes.

HTML uses the same provider-specific Page Agent conversations, ordinary image attachments, supported controls, queue/busy behavior, Stop, archives, interactions, and failure handling as Markdown. Features remain capability-dependent: not every provider exposes skills, compaction, or archive deletion. The parent indexes at most 128 eligible HTML components from inert exact source. Automatic kinds are labeled section/article/region, meaningful image/figure, labeled SVG/canvas chart, table, a `pre` containing `code`, and blockquote/note/alert; custom components are explicit-only. Each candidate has one unique stable source ID, a label, a source-order ordinal, and an exact-source excerpt of at most 48 KiB.
...
The browser protocol uses WebSocket subprotocol `agent-whiteboard.v5`. Connect, submit, and New commands always carry a nullable `settings` field. Settings-capable Pi, Codex, and Cursor sessions use the complete semantic tuple `{model, effort, speed}`. Pi fixes the semantic speed to `"standard"` and does not render a Speed control; Codex renders Speed when its catalog advertises the choice. Cursor's model catalog contains exact complete variants discovered through public `cursor-agent --list-models`; its compatibility tuple uses singleton Default effort and Standard speed, but the browser renders only the searchable Model menu because any reasoning or Fast attribute is already part of the native variant. Cursor derives image capability from ACP. Snapshots and settings events carry a strict visible model catalog plus either verified effective settings or an explicit unverified state. Catalogs accept at most 256 models within the existing 128 KiB bound. Snapshots and lifecycle events use nullable `active_work` with `turn|compact` kind and `running|stopping` state. Every strict snapshot also carries provider-neutral `skills_state`, a bounded safe `skills` catalog, nullable `max_selected_skills`, `supports_compact`, `busy_policy`, and broker-derived `composer_admission`. Pi advertises at most one selected skill and queue admission; Codex advertises its own skill limit and preserve-draft busy behavior. Cursor advertises preserve-draft busy behavior, no native skill catalog, no manual compaction, and no archive deletion; its archives can list and restore through standard `session/load`. Queue items carry their immutable captured settings, and an accepted-turn settings event is the only browser-preference update authority.

When the provider reports that the saved current native session is missing, Connect can succeed for conversation management: the snapshot has `lifecycle: "unavailable"` and `composer_admission: "blocked"`, followed by a `native_session_missing` error event. No model turn is submitted and no replacement is created by Connect. The existing `new`, archive, and resync commands remain available; native-session operations are rejected. Explicit `new` atomically archives the old reference, including any unresolved submission metadata, and installs the new current using the existing handoff and stale-reference checks. Nullable New settings allow provider defaults even when the old session's effective settings cannot be verified. No browser protocol version or durable schema change is required.

Submit, queue, history, and user-message payloads carry `content.parts`: ordered text parts interleaved with revision-bound references and safe skill invocations. Native skill paths, bodies, errors, dependencies, icons, and URLs never cross this boundary. `compact` commands carry only an opaque `work_id`; generalized interrupt commands target that advertised work ID. Skill-catalog and compaction-status events are replayable only for the existing broker actor lifetime. Image commands and events contain only bounded opaque references/descriptors—never bytes or private filesystem paths. Cursor submission opens validated workspace files with Unix descriptor and binding checks, then sends base64 standard ACP image blocks. New provider submissions use native envelope v4, including ordered HTML component references. Historical native v1, v2, and v3 envelopes remain parseable and are normalized, so provider-native Pi, Codex, and Cursor histories are preserved. Cursor history is reconstructed only from validated exact envelope replay before successful `session/load`, with broker-safe IDs and timestamps; failure or reconnect never automatically resubmits a turn. Browser/local protocol v5 is intentionally incompatible with older viewers and brokers: mixed versions fail closed rather than adapting. Deploy viewer and broker together, clear only disposable local broker conversation/workspace state, and restart `agent serve` before reconnecting. Public whiteboards and provider-native Pi, Codex, and Cursor histories are outside that reset.
