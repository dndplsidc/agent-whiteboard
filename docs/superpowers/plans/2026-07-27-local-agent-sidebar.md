...
- Chromium does not generically block a script-capable sandbox from navigating its own frame. The application-controlled wrapper's `frame-src 'self'` currently limits that navigation to the publishing origin, while `credentialless` and no-referrer policy strip publishing-origin cookies and Referer. Submitted scripts and links may therefore send a permitted same-origin request and deliberately encode the inner capability URL for publishing-origin handlers or logs; cross-origin child navigation is additionally blocked by the wrapper CSP. Direct top-level `/content` navigation remains independently opaque and storage/network restricted but does not inherit the wrapper's destination restriction. This accepted disclosure risk keeps standalone HTML a trusted-active-content feature; it must never render untrusted code. The opaque origin and broker Origin checks prevent an authorized broker connection.
...
Standalone HTML becomes opaque-origin active content so trusting a publishing origin does not grant submitted JavaScript that origin's credentials, storage, parent access, or broker authorization. Submitted JavaScript remains trusted code and retains the accepted ability to navigate its own sandboxed frame; the wrapper limits that destination to self and strips credentials/referrer, but code can deliberately disclose its capability to publishing-origin handlers or logs.
...
- Sandboxing all standalone HTML intentionally changes existing rendering authority. Stable capability URLs and migration documentation reduce disruption, but preserving unrestricted same-origin behavior is not compatible with the approved security boundary. Chromium permits self-navigation while allowing arbitrary scripts; the wrapper restricts the destination to the publishing origin and strips credentials/referrer, but trusted standalone code may explicitly disclose its capability URL to publishing-origin handlers or logs. Direct top-level `/content` navigation does not inherit the outer destination restriction. These accepted limitations must be documented and regression-tested.
