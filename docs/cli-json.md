...
Cursor runs `cursor-agent acp` over stable standard ACP v1, one child per loaded conversation; readiness may use a transient non-conversation probe. Negotiation requires ACP v1 and stable `session/list` and `session/load`, and fails closed otherwise. Cursor-private `cursor/*` methods, private storage, ACP client tools, filesystem or terminal capabilities, and arbitrary configuration options are outside the supported surface.
