...
Whiteboard Markdown, creator context, metadata, and reader messages are untrusted model input. Codex runs with the user's normal effective Codex home and inherits the configured model, tools, MCP servers, apps, hooks, skills, project instructions, approval policy, sandbox, and other settings unchanged. Agent Whiteboard does not create a production `CODEX_HOME`, edit `config.toml`, or add per-turn model, approval, sandbox, or tool overrides. One lazily started App Server process is shared by loaded Codex conversations; provider failures are isolated where possible, but an App Server process failure can interrupt all active Codex turns.

The effective Codex approval and sandbox policy—not Page Agent—authorizes tool execution. Prompt injection in published content can therefore cause tool activity or approval requests. If the effective approval policy does not request confirmation, Page Agent does not add one; in particular, `approval_policy = "never"` can allow actions within the effective sandbox without a browser prompt. A browser admitted to the local broker can see the bounded tool and approval details for its attached conversation.

This initial non-public deployment explicitly defers tool allowlists, content-only Codex execution, per-whiteboard filesystem roots, container or VM isolation, experimental permission profiles, and a stronger cross-agent sandbox. These are trust risks, not security guarantees. Use a suitably restrictive Codex configuration and sandbox for the content you open, and do not treat the `agent.default_access: content-only` setting as a Codex tool boundary.
