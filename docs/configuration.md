...
Page Agent builds only controls supported by each provider. Pi and Codex retain their existing model, effort, skill, compaction, and provider-specific speed behavior. Cursor projects Model from standard ACP configuration options, fixes Effort to the singleton Default choice and Speed to Standard, and obtains image capability from ACP. Cursor has preserve-draft busy behavior and does not expose a native skill catalog, manual compaction, custom Cursor UI, or native archive deletion. Its archives can be listed and restored through stable `session/load`. Exact `/new` starts a fresh provider-scoped conversation after confirmation. Existing and restored conversations inherit their effective native tuple; a genuinely new conversation may use that provider's same-origin last-accepted preference. Applying Pi settings follows normal Pi behavior: it changes the current Page Agent Pi session and Pi's future default, not already-running Pi sessions. App Server experimental APIs remain disabled for Codex.

Cursor uses stable standard ACP v1. Agent Whiteboard launches `cursor-agent acp`, one child for each loaded Cursor conversation; readiness may start a transient probe that is not a conversation. Cursor availability fails closed unless ACP v1 negotiates successfully and stable `session/list` and `session/load` are present. Agent Whiteboard does not use Cursor-private `cursor/*` methods, private files or storage, ACP client tools, filesystem or terminal capabilities, or arbitrary configuration options.
