...
Pi and Codex inherit the foreground process environment so providers use the normal effective user home, authentication, tools, extensions, apps, hooks, skills, approval policy, sandbox, project trust, and other native configuration. Agent Whiteboard does not set a production `CODEX_HOME`, edit provider configuration files, copy authentication, or override approval, sandbox, tools, or other native settings. For Codex only, Page Agent may apply a model, one advertised reasoning effort, and Standard/Fast service speed through native thread and turn fields. Existing and restored conversations inherit their effective native tuple; the selected tuple for a genuinely new conversation may come from the same-origin browser's last accepted preference. App Server experimental APIs remain disabled for Codex.
