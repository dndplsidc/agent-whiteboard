...
For NVM, activate the intended Node version before running `agent-whiteboard agent serve --daemon`. For an ephemeral Nix development environment, run installation from inside `nix develop`. Their active absolute `bin` directories are then recorded alongside stable Nix profile locations. Rerun the installation command after switching or removing a version-managed runtime; installation regenerates and reloads the plist. A provider script that uses `/usr/bin/env` is accepted only when its interpreter is executable through the generated LaunchAgent `PATH`, so an incomplete runtime fails installation with actionable guidance instead of failing later during provider startup. The LaunchAgent still does not copy provider configuration, authentication, credentials, or unrelated environment variables. Managed providers therefore use the same effective default user configuration as foreground providers, without editing provider configuration files.
