...
During a response, Stop and queued follow-up submission remain available together. A three-dot responding indicator appears only after the broker reports the authoritative responding lifecycle, then real streamed deltas replace it progressively. Context and provider-specific archives are alternate pane views.

For Codex, the compact composer pill selects a live-catalog model, one of that model's advertised reasoning efforts, and Standard or Fast speed. Standard maps to native `serviceTier: null` (reported as `default`) and Fast to native `serviceTier: "priority"`. Each submit captures its exact tuple, so queued messages retain different settings through edits and FIFO execution. The conversation default changes only after native acceptance; rejection or stale catalog drift preserves the draft for retry. New conversation uses the visible pill, while existing and restored conversations keep their own native effective settings. The last accepted tuple is stored only in browser storage for genuinely new Codex conversations on the same origin; unsent edits are tab-local and disappear on reload.
