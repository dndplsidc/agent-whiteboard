...
Refreshing resumes the saved current conversation for that page and provider; there is no shared default thread ID. New Codex conversations are given the title **Page Agent** and their empty history is persisted and checked before the reference is saved, without sending a model message. If an older saved thread is missing, the pane shows **Conversation unavailable** and offers **Start new conversation**. Confirming keeps the old reference in Archives and makes the new conversation current. It does not reconstruct unavailable messages or silently replace the old thread.
