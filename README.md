...
`agent serve` resolves `pi` and `codex` independently from `PATH`. Use `--pi-executable PATH` or `AGENT_WHITEBOARD_PROVIDER_PI_EXECUTABLE` for Pi, and `--codex-executable PATH` or `AGENT_WHITEBOARD_PROVIDER_CODEX_EXECUTABLE` for Codex. If either executable is unavailable, the other provider and the local broker still work. The Pi adapter uses Pi's provider-native login state and deliberately excludes ambient provider API-key and auth-token variables from the Pi process environment.

Codex runs through one lazily started `codex app-server` process shared by the loaded Codex conversations. The process inherits the effective default user Codex home, authentication, model, tools, MCP servers, apps, hooks, skills, approval policy, sandbox, and other configuration unchanged. Agent Whiteboard never edits `~/.codex/config.toml`, creates a production `CODEX_HOME`, or supplies per-turn model, approval, sandbox, or tool overrides. The broker binds only to literal IPv4 loopback and admits exact configured HTTPS origins.
...
Browser preferences are limited to theme, pane-open state, decimal loopback port, validated pane width, and the selected provider (`pi` or `codex`). Messages, context, capabilities, IDs, consent, conversation state, approval requests and decisions, provider output, credentials, and hidden reasoning are never stored there.

Codex whiteboard content is untrusted model input and the user's effective Codex tools, approval policy, and sandbox remain authoritative. In particular, `approval_policy = "never"` may permit tool actions without a Page Agent prompt. Tool allowlists, per-whiteboard filesystem roots, and a stronger cross-agent sandbox are deferred for this non-public deployment; do not treat Page Agent as a content-only execution boundary for Codex.

On macOS, install and start the managed per-user LaunchAgent with `agent-whiteboard agent serve --daemon`. It records the absolute agent and configuration paths plus the resolved Pi and Codex executable paths when available during installation; provider configuration and credentials are never copied. The child reads Agent Whiteboard configuration at startup and Codex still uses the effective default user configuration unchanged. Inspect the daemon with `agent-whiteboard agent daemon status`, reload it with `restart`, stop it with `stop` (the plist remains installed), or remove it with `uninstall`. Daemon operations are unsupported on Linux; run `agent-whiteboard agent serve` in the foreground instead. Authenticate with each provider itself; Agent Whiteboard never stores or accepts provider credentials.
