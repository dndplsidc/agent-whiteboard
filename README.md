...
When `viewer.local_agent.enabled` is true, Markdown viewers show an Agent control. Current Chrome checks the configured loopback port (default `8568`) and may require Local Network Access permission. Opening the drawer or checking status sends no page content. The reader must select **Connect**; the first message then sends the complete Markdown, creator context, title, URL, and resource metadata to local Pi in content-only mode. Reconnects resume broker state but never resubmit an interrupted or uncertain turn. The drawer exposes current context, streaming output, the shared follow-up queue, Stop, and local conversation archives. It stores only its open state and decimal port; messages, context, IDs, provider output, and credentials are not stored in browser preferences.
