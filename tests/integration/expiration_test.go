...
func TestExpirationOmittedAndPermanentCreation(t *testing.T) {
	...
	omitted := runCLIResource(t, server, "--json", "create", "markdown", "--context", creatorContext, "--title", "Expiring board", "--summary", "Uses the server default expiration", file)
	require.False(t, omitted.Resource.Permanent)
	require.NotNil(t, omitted.Resource.ExpiresAt)
	permanent := runCLIResource(t, server, "--json", "create", "markdown", "--context", creatorContext, "--title", "Permanent board", "--summary", "Never expires", "--expires-in", "0", file)
...
func TestExpirationUpdatePreservesOrMovesAbsoluteDeadline(t *testing.T) {
	...
	created := runCLIResource(t, server, "--json", "create", "markdown", "--context", creatorContext, "--title", "Deadline board", "--summary", "Exercises expiration updates", "--expires-in", "30", first)
...
func TestExpirationRejectsNegativeAndCannotReviveExpired(t *testing.T) {
	...
	stdout, stderr, err := server.RunCLI(ctx, "--json", "create", "markdown", "--context", creatorContext, "--title", "Invalid expiration", "--summary", "Must be rejected", "--expires-in=-1", file)
	cancel()
	require.Error(t, err)
	...
	created := runCLIResource(t, server, "--json", "create", "markdown", "--context", creatorContext, "--title", "Short-lived board", "--summary", "Expires before update", "--expires-in", "1", file)
