...
Standalone HTML must be a head-first document with no external script or stylesheet dependencies. Prefer semantic selectable elements with unique stable IDs and native accessible labels. Automatic discovery covers labeled sections/regions, meaningful images/figures, labeled SVG/canvas charts, tables, `pre` elements containing `code`, and blockquote/note/alert content. Use `data-agent-select="section|image|chart|table|code|quote|component"` or `data-agent-section` only for ambiguity, and `data-agent-select="none"` or `data-agent-section-ignore` for intentional exclusion. Create/update rejects malformed explicit declarations, including conflicts, unsupported values, invalid or duplicate IDs, excluded explicit components, and missing labels. Never embed Agent Whiteboard bridge code or claim video/audio, runtime pixels, or live state will be captured.
...
Page Agent browser/local API v5 uses WebSocket subprotocol `agent-whiteboard.v5`; new provider turns use native envelope v4 while native v1-v3 histories remain readable. Deploy v5 viewer and broker together and reset only disposable local broker conversation/workspace state. Page Agent provider selection sends no content and keeps Pi and Codex conversations, lifecycle state, pending requests, and archives separate. Pi keeps its queue; Codex preserves an editable busy draft but admits no follow-up until idle. Codex receives the complete canonical context envelope as user-message content. Its enabled native skills are selectable with `$`, and exact `/compact` invokes stable native manual compaction; skill paths and bodies never enter the browser protocol. Bounded tool activity and stable command, file-change, permission, and MCP elicitation requests are relayed through the browser protocol; the first valid response across attached tabs wins. App Server `request_user_input` is experimental, so `experimentalApi` remains disabled and that interaction is not active.
