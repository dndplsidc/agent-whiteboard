...
Page Agent provider selection sends no content and keeps Pi and Codex conversations, lifecycle state, pending requests, and archives separate. Pi keeps its queue; Codex preserves an editable busy draft but admits no follow-up until idle. Codex receives the complete canonical context envelope as user-message content. Its enabled native skills are selectable with `$`, and exact `/compact` invokes stable native manual compaction; skill paths and bodies never enter the browser protocol. Bounded tool activity and stable command, file-change, permission, and MCP elicitation requests are relayed through the browser protocol; the first valid response across attached tabs wins. App Server `request_user_input` is experimental, so `experimentalApi` remains disabled and that interaction is not active.
