...
Pi uses its provider-native login state rather than ambient API-key environment variables. Codex uses the effective default user Codex home, authentication, model, tools, MCP, apps, hooks, skills, approval policy, sandbox, and other configuration unchanged; Agent Whiteboard never edits `~/.codex/config.toml` or creates a production `CODEX_HOME`. Tool allowlists, per-whiteboard filesystem roots, and a stronger cross-agent sandbox are deferred, so treat whiteboard content as untrusted model input and do not claim a content-only Codex execution boundary. Never invent Agent Whiteboard authentication commands or treat daemon management as provider authentication.
