...
// NewLaunchAgentManager constructs the macOS per-user LaunchAgent manager.
func NewLaunchAgentManager(runner LaunchAgentRunner) (LaunchAgentManager, error) {
	if nilRunner(runner) {
...
func newDarwinManager(runner LaunchAgentRunner, home string, uid int, ops fileOps) *darwinManager {
	return &darwinManager{runner: runner, home: home, uid: uid, paths: pathsForHome(home), ops: ops}
}

func nilRunner(runner LaunchAgentRunner) bool {
	if runner == nil {
		return true
	}
...
func (manager *darwinManager) Install(ctx context.Context, config LaunchAgentConfig) error {
	manager.mu.Lock()
	...
	if err := manager.verifyPublicationBindings(launchAgents, logs, stdoutLog, stderrLog, publication); err != nil {
		if rollbackErr := publication.rollback(); rollbackErr != nil {
			return &LaunchAgentCommitUncertainError{Err: errors.Join(err, rollbackErr)}
		}
		return err
	}
	...
	if err := manager.verifyPublicationBindings(launchAgents, logs, stdoutLog, stderrLog, publication); err != nil {
		return &LaunchAgentCommitUncertainError{Err: err}
	}

	target := manager.target()
	output, printErr := manager.runner.Run(ctx, LaunchAgentControlExecutable, "print", target)
	if printErr == nil {
		if output, bootoutErr := manager.runner.Run(ctx, LaunchAgentControlExecutable, "bootout", target); bootoutErr != nil && !notLoaded(output, bootoutErr) {
	...
	if _, err := manager.runner.Run(ctx, LaunchAgentControlExecutable, "bootstrap", manager.domain(), manager.paths.Plist); err != nil {
		return reloadFailure("bootstrap", err)
	}
	if _, err := manager.runner.Run(ctx, LaunchAgentControlExecutable, "kickstart", "-k", target); err != nil {
...
func (manager *darwinManager) Status(ctx context.Context) (LaunchAgentStatus, error) {
	manager.mu.Lock()
	defer manager.mu.Unlock()
	...
	status := LaunchAgentStatus{Installed: installed}
	output, err := manager.runner.Run(ctx, LaunchAgentControlExecutable, "print", manager.target())
	if err != nil {
...
func (manager *darwinManager) Restart(ctx context.Context) error {
	...
	if !installed {
		return ErrLaunchAgentNotInstalled
	}
	target := manager.target()
	if output, err := manager.runner.Run(ctx, LaunchAgentControlExecutable, "bootout", target); err != nil && !notLoaded(output, err) {
...
func (manager *darwinManager) Stop(ctx context.Context) error {
	...
	output, err := manager.runner.Run(ctx, LaunchAgentControlExecutable, "bootout", manager.target())
...
func (manager *darwinManager) target() string {
	return manager.domain() + "/" + LaunchAgentLabel
}

func reloadFailure(operation string, cause error) error {
	return errors.Join(ErrLaunchAgentInstalledNotRunning, fmt.Errorf("launchctl %s failed: %w", operation, cause))
}
