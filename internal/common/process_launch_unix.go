...
func (*ProcessGroupLauncher) Launch(ctx context.Context, request ProcessRequest) (ManagedProcess, error) {
	if err := ctx.Err(); err != nil {
		return nil, processError(ProcessErrorCanceled, "launch", err)
	}
	if err := request.Validate(); err != nil {
		return nil, processError(ProcessErrorInvalidRequest, "validate", err)
	}
	if err := validateExecutable(request.Executable); err != nil {
		return nil, processError(ProcessErrorExecutable, "validate executable", err)
	}
	if err := validateWorkingDirectory(request.WorkingDirectory); err != nil {
		return nil, processError(ProcessErrorWorkingDirectory, "validate working directory", err)
	}

	pipes, err := openChildPipes()
	if err != nil {
		return nil, processError(ProcessErrorStart, "create pipes", err)
	}
	closeAll := true
	defer func() {
		if closeAll {
	...
	if err := command.Start(); err != nil {
		return nil, processError(ProcessErrorStart, "start", err)
	}
	...
	if err != nil || pgid != pid {
		...
		return nil, processError(ProcessErrorUnsafeProcessGroup, "verify child process group", err)
	}
	brokerPGID := unix.Getpgrp()
	if pgid == brokerPGID {
		cleanupStartedProcess(command, pid)
		return nil, processError(ProcessErrorUnsafeProcessGroup, "verify child process group", errors.New("child process group matches broker process group"))
	}

	identity, err := newProcessIdentity(pid)
	if err != nil {
		cleanupStartedProcess(command, pid)
		return nil, processError(ProcessErrorUnsafeProcessGroup, "track child process", err)
	}
	child := &managedChild{
		command:    command,
		input:      pipes.input,
...
func monitorProcessError(err error) error {
	...
	return processError(ProcessErrorStart, "monitor child", err)
}
...
func (child *managedChild) Wait() error {
	if child.done == nil {
		return processError(ProcessErrorStart, "wait", errors.New("child was not started"))
	}
...
func (child *managedChild) signalGroupLocked(value unix.Signal, operation string, missingIsSuccess bool) error {
	if child.pid <= 0 || child.pgid <= 0 || child.pgid != child.pid || child.pgid == child.brokerPGID || child.pgid == unix.Getpgrp() {
		return processError(ProcessErrorUnsafeProcessGroup, operation, errors.New("refusing to signal an unverified or broker process group"))
	}
	if err := child.signalFn(-child.pgid, value); err != nil {
		if missingIsSuccess && errors.Is(err, unix.ESRCH) {
			return nil
		}
		return processError(ProcessErrorSignal, operation, err)
	}
	return nil
}

var _ ManagedProcess = (*managedChild)(nil)
