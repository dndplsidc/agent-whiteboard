...
func TestInstallWritesPrivateFilesAndUsesExactLaunchctlSequence(t *testing.T) {
	home := testHome(t)
	runner := &fakeRunner{results: []runnerResult{{err: ErrLaunchAgentNotLoaded}, {}, {}}}
	...
	assertCalls(t, runner.calls, []runnerCall{
		{command: LaunchAgentControlExecutable, args: []string{"print", "gui/501/" + LaunchAgentLabel}},
		{command: LaunchAgentControlExecutable, args: []string{"bootstrap", "gui/501", paths.Plist}},
		{command: LaunchAgentControlExecutable, args: []string{"kickstart", "-k", "gui/501/" + LaunchAgentLabel}},
	})
}
...
func TestInstallPreservesExistingLibraryMode(t *testing.T) {
	...
	manager := newDarwinManager(&fakeRunner{results: []runnerResult{{err: ErrLaunchAgentNotLoaded}, {}, {}}}, home, 501, defaultFileOps())
...
func TestInstallUpdateBootsOutLoadedAgentBeforeReload(t *testing.T) {
	...
	paths := pathsForHome(home)
	assertCalls(t, runner.calls, []runnerCall{
		{command: LaunchAgentControlExecutable, args: []string{"print", "gui/502/" + LaunchAgentLabel}},
		{command: LaunchAgentControlExecutable, args: []string{"bootout", "gui/502/" + LaunchAgentLabel}},
		{command: LaunchAgentControlExecutable, args: []string{"bootstrap", "gui/502", paths.Plist}},
		{command: LaunchAgentControlExecutable, args: []string{"kickstart", "-k", "gui/502/" + LaunchAgentLabel}},
	})
}
...
func TestInstallRejectsSymlinkedLaunchAgentsDirectoryWithoutInvokingRunner(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(outside, LaunchAgentLabel+".plist")); !errors.Is(err, os.ErrNotExist) {
...
func TestInstallReloadFailureIsClassifiedAndKeepsDurablePlist(t *testing.T) {
	home := testHome(t)
	runner := &fakeRunner{results: []runnerResult{{err: ErrLaunchAgentNotLoaded}, {err: errors.New("bootstrap failed")}}}
	...
	if !errors.Is(err, ErrLaunchAgentInstalledNotRunning) {
...
func TestStatusCombinesPlistExistenceAndRedactedLaunchctlState(t *testing.T) {
	...
	want := LaunchAgentStatus{Installed: true, Loaded: true, Running: true, PID: 9182}
	if status != want {
	...
	assertCalls(t, runner.calls, []runnerCall{{command: LaunchAgentControlExecutable, args: []string{"print", "gui/505/" + LaunchAgentLabel}}})
}
...
func TestStatusDistinguishesNotLoadedFromArbitraryFailure(t *testing.T) {
	...
	notLoadedRunner := &fakeRunner{results: []runnerResult{{err: ErrLaunchAgentNotLoaded}}}
	manager := newDarwinManager(notLoadedRunner, home, 506, defaultFileOps())
	status, err := manager.Status(context.Background())
	if err != nil {
		t.Fatal(err)
	}
	if status != (LaunchAgentStatus{Installed: true}) {
...
func TestRestartRequiresInstalledPlistAndUsesExactSequence(t *testing.T) {
	...
	if err := manager.Restart(context.Background()); !errors.Is(err, ErrLaunchAgentNotInstalled) {
	...
	assertCalls(t, runner.calls, []runnerCall{
		{command: LaunchAgentControlExecutable, args: []string{"bootout", "gui/507/" + LaunchAgentLabel}},
		{command: LaunchAgentControlExecutable, args: []string{"bootstrap", "gui/507", paths.Plist}},
		{command: LaunchAgentControlExecutable, args: []string{"kickstart", "-k", "gui/507/" + LaunchAgentLabel}},
	})
}
...
func TestStopAndUninstallAreIdempotent(t *testing.T) {
	...
	wantCall := runnerCall{command: LaunchAgentControlExecutable, args: []string{"bootout", "gui/508/" + LaunchAgentLabel}}
...
func TestOwnershipMismatchAndUncertainRecoveryAreRejected(t *testing.T) {
	...
	var uncertain *LaunchAgentCommitUncertainError
	if !errors.As(err, &uncertain) {
		t.Fatalf("error = %v, want LaunchAgentCommitUncertainError", err)
	}
	if len(runner.calls) != 0 {
...
func TestCommitBackupRemovalSyncFailureIsUncertain(t *testing.T) {
	home := testHome(t)
	paths := pathsForHome(home)
	initialRunner := &fakeRunner{results: []runnerResult{{err: ErrLaunchAgentNotLoaded}, {}, {}}}
	...
	var uncertain *LaunchAgentCommitUncertainError
	if !errors.As(err, &uncertain) {
		t.Fatalf("install error = %v, want LaunchAgentCommitUncertainError", err)
	}
...
func TestCommitBackupRemovalSyncFailureEndsRollbackState(t *testing.T) {
	...
	var uncertain *LaunchAgentCommitUncertainError
	if !errors.As(err, &uncertain) {
		t.Fatalf("commit error = %v, want LaunchAgentCommitUncertainError", err)
	}
...
func TestRemoveDurableRestoresPriorWhenConditionalRemovalCannotRestoreTombstone(t *testing.T) {
	...
	var uncertain *LaunchAgentCommitUncertainError
	if !errors.As(err, &uncertain) {
		t.Fatalf("remove error = %v, want LaunchAgentCommitUncertainError", err)
	}
	assertFileContents(t, paths.Plist, prior)
}
...
func TestNewManagerUsesTemporaryHOMEAndNeverRealLaunchAgents(t *testing.T) {
	...
	t.Setenv("HOME", home)
	runner := &fakeRunner{results: []runnerResult{{err: ErrLaunchAgentNotLoaded}, {}, {}}}
	manager, err := NewLaunchAgentManager(runner)
	if err != nil {
		t.Fatal(err)
	}
...
func validTestConfig(t *testing.T, home, suffix string) LaunchAgentConfig {
	t.Helper()
