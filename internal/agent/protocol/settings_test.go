...
func TestV4QueueSnapshotAndSettingsEventsCarryBoundedPresentationAndCatalog(t *testing.T) {
	...
	payloads := []protocol.EventPayload{
		...
		protocol.SnapshotPayload{
			...
			SettingsState: &state, EffectiveSettings: &settings, Catalog: catalog,
		},
	...
	piSnapshot := protocol.SnapshotPayload{Lifecycle: protocol.LifecycleReady, Queue: []protocol.QueueItem{}, ContextState: protocol.ContextAccepted, Catalog: []protocol.CatalogModel{}}
...
func TestV4SettingsEventsRejectContradictoryAndMalformedState(t *testing.T) {
	...
	invalid := []protocol.EventPayload{
		...
		protocol.SnapshotPayload{Lifecycle: protocol.LifecycleReady, Queue: []protocol.QueueItem{}, ContextState: protocol.ContextAccepted, SettingsState: &state, Catalog: catalog},
		protocol.SnapshotPayload{Lifecycle: protocol.LifecycleReady, Queue: []protocol.QueueItem{}, ContextState: protocol.ContextAccepted, SettingsState: &state, EffectiveSettings: &settings, Catalog: catalog},
	}
...
func TestV4CatalogWorstCaseFitsMeasuredEventBound(t *testing.T) {
	...
	state := protocol.SettingsVerified
	payload := protocol.SnapshotPayload{Lifecycle: protocol.LifecycleReady, Queue: queue, ContextState: protocol.ContextAccepted, SettingsState: &state, EffectiveSettings: &settings, Catalog: catalog}
