...
func (b *queuedBackend) Connect(context.Context, string, protocol.Command) (broker.BrowserConnection, error) {
	b.mu.Lock()
	defer b.mu.Unlock()
...
type controlledConnection struct {
	...
	commandErr     error
	commandResult  *protocol.Event
	mu             sync.Mutex
	commands       int
	closeCalls     int
}

func newControlledConnection(events ...protocol.Event) *controlledConnection {
	channel := make(chan protocol.Event, len(events))
...
func (c *controlledConnection) Events() <-chan protocol.Event { return c.events }
...
func (c *controlledConnection) Command(ctx context.Context, _ protocol.Command) (protocol.Event, error) {
	c.mu.Lock()
	c.commands++
	...
	if commandErr != nil {
		return protocol.Event{}, commandErr
	}
...
func TestFallbackReconnectDoesNotReplaceBeforeFirstEvent(t *testing.T) {
	...
	request := func(ctx context.Context) (*http.Response, error) {
		req, err := http.NewRequestWithContext(ctx, http.MethodPost, baseURL+protocol.ConnectPath, bytes.NewReader(body))
		if err != nil {
			return nil, err
		}
		req.Header.Set("Origin", trustedOrigin)
		req.Header.Set("Content-Type", "application/json")
		req.Header.Set(protocol.APIVersionHeader, protocol.APIVersion)
	...
	case <-firstConnection.closed:
		t.Fatal("working browserAttachment closed before reconnect produced its first event")
	default:
	}
	...
	commandResponse.Header.Set(protocol.APIVersionHeader, protocol.APIVersion)
...
func TestFallbackInvalidFirstEventLeavesOldAttachmentUsable(t *testing.T) {
	...
	first := post(protocol.ConnectPath, connectCommand())
	_, err := bufio.NewReader(first.Body).ReadBytes('\n')
	require.NoError(t, err)
	failed := post(protocol.ConnectPath, connectCommand())
	require.Equal(t, http.StatusServiceUnavailable, failed.StatusCode)
	failed.Body.Close()

	select {
	case <-firstConnection.closed:
		t.Fatal("invalid reconnect event closed the working browserAttachment")
	default:
	}
...
func TestRemovedTrustPreservesAttachmentAcrossCommandStreamRace(t *testing.T) {
	...
	streamRequest, err := http.NewRequestWithContext(streamCtx, http.MethodPost, running.baseURL+protocol.ConnectPath, bytes.NewReader(encodeCommand(t, connectCommand())))
	require.NoError(t, err)
	streamRequest.Header.Set("Origin", trustedOrigin)
	streamRequest.Header.Set("Content-Type", "application/json")
	streamRequest.Header.Set(protocol.APIVersionHeader, protocol.APIVersion)
...
func TestAttachmentRegistrySerializesActivationWithCommandDispatch(t *testing.T) {
	...
	defer oldCancel()
	defer newCancel()
	old := &browserAttachment{key: key, connection: newSafeConnection(oldConnection), ctx: oldCtx, cancel: oldCancel, done: make(chan struct{})}
	newItem := &browserAttachment{key: key, connection: newSafeConnection(newConnection), ctx: newCtx, cancel: newCancel, done: make(chan struct{})}
...
func TestFallbackReconnectHandoffWaitsForOldCommandAndDispatchesNewCommandsToNewConnection(t *testing.T) {
	...
	post := func(ctx context.Context, path string, command protocol.Command) (*http.Response, error) {
	...
	first, err := post(context.Background(), protocol.ConnectPath, connectCommand())
	require.NoError(t, err)
	_, err = bufio.NewReader(first.Body).ReadBytes('\n')
	require.NoError(t, err)
	...
	go func() {
		response, requestErr := post(context.Background(), protocol.ConnectPath, connectCommand())
		if requestErr != nil {
...
func TestFallbackFirstEventRequiresSnapshotUnlessReplayRequested(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			request, err := http.NewRequest(http.MethodPost, baseURL+protocol.ConnectPath, bytes.NewReader(encodeCommand(t, connect)))
			require.NoError(t, err)
			request.Header.Set("Origin", trustedOrigin)
			request.Header.Set("Content-Type", "application/json")
			request.Header.Set(protocol.APIVersionHeader, protocol.APIVersion)
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			if test.accepted {
				...
				decoded, decodeErr := protocol.DecodeEvent(event)
				require.NoError(t, decodeErr)
				require.Equal(t, protocol.EventCommandResult, decoded.Type)
				return
			}
...
func TestServerCloseCancelsCooperativeFallbackCommand(t *testing.T) {
	...
	streamRequest, err := http.NewRequest(http.MethodPost, baseURL+protocol.ConnectPath, bytes.NewReader(encodeCommand(t, connectCommand())))
	require.NoError(t, err)
	streamRequest.Header.Set("Origin", trustedOrigin)
	streamRequest.Header.Set("Content-Type", "application/json")
	streamRequest.Header.Set(protocol.APIVersionHeader, protocol.APIVersion)
