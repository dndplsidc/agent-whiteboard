...
func TestStrictMutationHeadersSchemaAndAttachmentIdentity(t *testing.T) {
	...
	request, err := http.NewRequest(http.MethodPost, running.baseURL+protocol.ConnectPath, bytes.NewReader(connectBody))
	...
	request.Header.Set(protocol.APIVersionHeader, "1")
	response, err = running.client.Do(request)
	require.NoError(t, err)
	assert.Equal(t, http.StatusUpgradeRequired, response.StatusCode)
	assert.NotContains(t, readJSON(t, response), string(connectBody))

	response = running.request(t, http.MethodPost, protocol.ConnectPath, trustedOrigin, []byte(`{"unknown":"native secret"}`))
	assert.Equal(t, http.StatusBadRequest, response.StatusCode)
	body := readJSON(t, response)
	assert.NotContains(t, body, "native secret")

	stream := running.request(t, http.MethodPost, protocol.ConnectPath, trustedOrigin, connectBody)
...
func TestWebSocketRequiresProtocolRejectsBinaryAndTimesOutFirstFrame(t *testing.T) {
	...
	withoutOrigin := websocket.Dialer{Subprotocols: []string{protocol.WebSocketSubprotocol}}
...
func TestFallbackReconnectReplacesAndDetachesOldStream(t *testing.T) {
	...
	first := running.request(t, http.MethodPost, protocol.ConnectPath, trustedOrigin, body)
	firstReader := bufio.NewReader(first.Body)
	_, err := firstReader.ReadBytes('\n')
	require.NoError(t, err)

	second := running.request(t, http.MethodPost, protocol.ConnectPath, trustedOrigin, body)
...
func TestTrustSourceFailureFailsClosedButStatusRemainsMinimal(t *testing.T) {
	...
	assert.Equal(t, map[string]any{"available": true, "api_version": protocol.APIVersion, "origin_trusted": false}, readJSON(t, response))
	response = running.request(t, http.MethodPost, protocol.ConnectPath, trustedOrigin, encodeCommand(t, connectCommand()))
	assert.Equal(t, http.StatusForbidden, response.StatusCode)
	response.Body.Close()

	loopbackOrigin := "http://127.0.0.1:4321"
	response = running.request(t, http.MethodGet, protocol.StatusPath, loopbackOrigin, nil)
	assert.Equal(t, http.StatusOK, response.StatusCode)
	assert.Equal(t, map[string]any{"available": true, "api_version": protocol.APIVersion, "origin_trusted": true}, readJSON(t, response))
	stream := running.request(t, http.MethodPost, protocol.ConnectPath, loopbackOrigin, encodeCommand(t, connectCommand()))
