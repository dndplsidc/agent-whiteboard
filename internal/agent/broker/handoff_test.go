...
func newCommand(commandID, clientID, conversationID string) protocol.Command {
	return protocol.Command{
		APIVersion: protocol.APIVersion, CommandID: commandID, ClientID: clientID,
		ConversationID: &conversationID, Type: protocol.CommandNew, Payload: protocol.EmptyPayload{},
	}
}

func restoreCommand(commandID, clientID, conversationID, archiveID string) protocol.Command {
	return protocol.Command{
		APIVersion: protocol.APIVersion, CommandID: commandID, ClientID: clientID,
		ConversationID: &conversationID, Type: protocol.CommandArchiveRestore,
		Payload: protocol.ArchiveReferencePayload{ArchiveID: archiveID},
	}
}
...
func TestCommandNewHandsOffTwoTabsAndPublishesExactDurableCurrent(t *testing.T) {
	...
	require.Equal(t, protocol.ArchivePayload{Action: protocol.ArchiveCreated, ArchiveID: oldID}, firstArchive.Payload)
...
func TestCommandArchiveRestoreUsesStableWorkspaceWithoutHistoryAndHandsOff(t *testing.T) {
	...
	require.Equal(t, protocol.ArchivePayload{Action: protocol.ArchiveRestored, ArchiveID: archiveID}, archiveEvent.Payload)
	require.Equal(t, result, receiveLifecycle(t, first.Events()))
...
func TestHandoffExactPreconditionRejectsInterveningDurableMapping(t *testing.T) {
	...
	requireCommandResult(t, answer.event, protocol.CommandRejected, protocol.ErrorStateRepairFailed)
...
func TestRestoreReconcilesPreparedArchiveWithoutSubmitting(t *testing.T) {
	...
	broker, state, driver, connection, _ := archiveFixtureWithMapping(t, 2240, func(mapping *statepkg.Mapping) {
		archived := mapping.Archives[1]
		revision := statepkg.Revision{Digest: "cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc", Revision: statepkg.RevisionInitial, SourceUpdatedAt: archived.UpdatedAt}
		archived.Observed = &revision
		archived.PreparedCommit = &statepkg.PreparedCommit{Revision: revision, TurnID: turnID, Phase: statepkg.CommitPrepared}
		mapping.Archives[1] = archived
	})
	defer broker.Close(context.Background())
	state.repairState.mu.Lock()
	state.reconciliations = []repairMutation{{outcome: statepkg.CommitApplied, apply: true}}
	state.repairState.mu.Unlock()
	candidate := newTurnSession("sessions/archive-two")
...
func TestRestorePromotesAcceptedArchiveWithoutSubmitting(t *testing.T) {
	...
	broker, state, driver, connection, _ := archiveFixtureWithMapping(t, 2280, func(mapping *statepkg.Mapping) {
		archived := mapping.Archives[1]
		revision := statepkg.Revision{Digest: "dddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd", Revision: statepkg.RevisionInitial, SourceUpdatedAt: archived.UpdatedAt}
		archived.Observed = &revision
		archived.PreparedCommit = &statepkg.PreparedCommit{Revision: revision, TurnID: turnID, Phase: statepkg.CommitAccepted}
		mapping.Archives[1] = archived
	})
	defer broker.Close(context.Background())
	state.repairState.mu.Lock()
	state.promotions = []repairMutation{{outcome: statepkg.CommitApplied, apply: true}}
	state.repairState.mu.Unlock()
	candidate := newTurnSession("sessions/archive-two")
	driver.mu.Lock()
	driver.resumeSessions = []provider.Session{nil, candidate}
...
func TestRestorePreparedRepairRejectsForeignPostRestoreMapping(t *testing.T) {
	...
	broker, state, driver, connection, _ := archiveFixtureWithMapping(t, 2300, func(mapping *statepkg.Mapping) {
		archived := mapping.Archives[1]
		revision := statepkg.Revision{Digest: "eeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee", Revision: statepkg.RevisionInitial, SourceUpdatedAt: archived.UpdatedAt}
		archived.Observed = &revision
		archived.PreparedCommit = &statepkg.PreparedCommit{Revision: revision, TurnID: turnID, Phase: statepkg.CommitPrepared}
		mapping.Archives[1] = archived
	})
...
func TestBrokerCloseDeadlineDoesNotBlockOnNoncooperativeCandidateCleanup(t *testing.T) {
	...
	answer := receiveLifecycle(t, response)
	require.NoError(t, answer.err)
	requireCommandResult(t, answer.event, protocol.CommandRejected, protocol.ErrorProviderProtocolFailure)
	require.NoError(t, broker.Close(context.Background()))
	require.EqualValues(t, 1, candidate.shutdowns.Load())
}
...
func TestConversationHandoffRejectsWhileProviderWorkerIsActive(t *testing.T) {
	...
	go func() {
		...
		_, _ = first.Command(context.Background(), historyCommand(sequenceID(2192), first.clientID, first.ConversationID(), protocol.PageRequestPayload{Limit: 1}))
	}()
