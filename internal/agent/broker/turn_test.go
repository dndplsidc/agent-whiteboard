...
func TestInteractiveRequestFirstResponseWinsAcrossAttachments(t *testing.T) {
	...
	nativeRequest := provider.InteractionRequest{
		...
		Command: "go test ./internal/agent/codex", WorkingDirectory: "/workspace",
	...
	for _, connection := range []*Connection{first, second} {
		...
		require.Equal(t, protocol.EventInteractionRequest, event.Type)
		require.Equal(t, requestID, event.Payload.(protocol.InteractionRequestPayload).RequestID)
	}
	...
	session.mu.Unlock()
	firstCommand := interactionResponseCommand(sequenceID(7056), firstClientID, conversationID, requestID, protocol.InteractionCommandApproval, "accept")
	firstResult := make(chan protocol.Event, 1)
	...
	secondCommand := interactionResponseCommand(sequenceID(7057), secondClientID, conversationID, requestID, protocol.InteractionCommandApproval, "decline")
	secondResult, err := second.Command(context.Background(), secondCommand)
	require.NoError(t, err)
	requireCommandResult(t, secondResult, protocol.CommandRejected, protocol.ErrorInvalidState)
	require.Equal(t, secondResult, receiveLifecycle(t, second.Events()))

	close(gate)
	require.NoError(t, <-firstError)
	requireCommandResult(t, <-firstResult, protocol.CommandSucceeded, "")
	for _, connection := range []*Connection{first, second} {
	...
	require.Equal(t, protocol.EventCommandResult, receiveLifecycle(t, first.Events()).Type)
	select {
...
func TestTerminalTurnExpiresPendingInteraction(t *testing.T) {
	...
	resolved := receiveLifecycle(t, connection.Events())
	require.Equal(t, protocol.EventInteractionResolved, resolved.Type)
	require.Equal(t, requestID, resolved.Payload.(protocol.InteractionResolvedPayload).RequestID)
	...
	response := interactionResponseCommand(sequenceID(7065), clientID, conversationID, requestID, protocol.InteractionCommandApproval, "accept")
	rejected, err := connection.Command(context.Background(), response)
	require.NoError(t, err)
	requireCommandResult(t, rejected, protocol.CommandRejected, protocol.ErrorInvalidState)
...
func TestProviderResolutionConsumesPendingInteractionAndIgnoresStaleDuplicate(t *testing.T) {
	...
	response := interactionResponseCommand(sequenceID(7095), clientID, conversationID, requestID, protocol.InteractionCommandApproval, "accept")
	rejected, err := connection.Command(context.Background(), response)
	require.NoError(t, err)
	requireCommandResult(t, rejected, protocol.CommandRejected, protocol.ErrorInvalidState)
	require.Equal(t, rejected, receiveLifecycle(t, connection.Events()))
...
func TestFailedNativeInteractionDeliveryStillConsumesTheFirstValidResponse(t *testing.T) {
	...
	response := interactionResponseCommand(sequenceID(7085), clientID, conversationID, requestID, protocol.InteractionCommandApproval, "accept")
	rejected, err := connection.Command(context.Background(), response)
	require.NoError(t, err)
	requireCommandResult(t, rejected, protocol.CommandRejected, protocol.ErrorProviderCrashed)
	require.Equal(t, requestID, receiveLifecycle(t, session.responded).RequestID)
	resolved := receiveLifecycle(t, connection.Events())
	require.Equal(t, protocol.EventInteractionResolved, resolved.Type)
	require.Equal(t, "accept", resolved.Payload.(protocol.InteractionResolvedPayload).OptionID)
	...
	second := interactionResponseCommand(sequenceID(7086), clientID, conversationID, requestID, protocol.InteractionCommandApproval, "decline")
...
func TestInteractionProviderStreamFailsClosedAtPendingCountLimit(t *testing.T) {
	...
	require.Equal(t, protocol.ErrorProviderMalformedStream, failure.Payload.(protocol.ErrorPayload).Error.Code())
...
func TestInteractionResponseValidationMatchesRequestSurface(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			response := protocol.InteractionResponsePayload{RequestID: test.request.ID, Kind: protocol.InteractionKind(test.request.Kind), OptionID: test.optionID, Answers: test.answers}
...
func TestInvalidInteractionResponseDoesNotReachOrConsumeProviderRequest(t *testing.T) {
	...
	invalid := interactionResponseCommand(sequenceID(7545), clientID, conversationID, requestID, protocol.InteractionCommandApproval, "other")
	...
	valid := interactionResponseCommand(sequenceID(7546), clientID, conversationID, requestID, protocol.InteractionCommandApproval, "accept")
...
func interactionResponseCommand(commandID, clientID, conversationID, requestID string, kind protocol.InteractionKind, optionID string) protocol.Command {
	return protocol.Command{
		APIVersion: protocol.APIVersion, CommandID: commandID, ClientID: clientID, ConversationID: &conversationID, Type: protocol.CommandInteractionRespond,
		Payload: protocol.InteractionResponsePayload{RequestID: requestID, Kind: kind, OptionID: optionID, Answers: map[string][]string{}},
	}
}
...
func TestSubmitCommitsContextOnceAndDeduplicatesTheCommandResult(t *testing.T) {
	...
	require.NoError(t, err)
	requireCommandResult(t, result, protocol.CommandSucceeded, "")
	require.Equal(t, command.Payload.(protocol.SubmitPayload).TurnID, receiveLifecycle(t, session.submitted).TurnID)

	contextEvent := receiveLifecycle(t, connection.Events())
	require.Equal(t, protocol.EventContext, contextEvent.Type)
	require.Equal(t, protocol.ContextAccepted, contextEvent.Payload.(protocol.ContextPayload).State)
	lifecycle := receiveLifecycle(t, connection.Events())
	require.Equal(t, protocol.EventLifecycle, lifecycle.Type)
	require.Equal(t, protocol.LifecycleResponding, lifecycle.Payload.(protocol.LifecyclePayload).State)
	...
	payload.Content = protocol.TextContent("different")
	changed.Payload = payload
	rejected, err := connection.Command(context.Background(), changed)
	require.NoError(t, err)
	requireCommandResult(t, rejected, protocol.CommandRejected, protocol.ErrorInvalidCommand)
	require.EqualValues(t, 1, session.submissions.Load())
	_ = identity
	_ = resource
}
...
func TestFollowUpQueueEditRemoveFIFOAndInterruptPreserveQueue(t *testing.T) {
	...
	require.Equal(t, []protocol.QueueItem{{TurnID: sequenceID(7111), MessageID: sequenceID(7112), Content: protocol.TextContent("first")}}, queueEvent.Payload.(protocol.QueuePayload).Items)
	require.Equal(t, result, receiveLifecycle(t, connection.Events()))
	...
	edit := protocol.Command{APIVersion: protocol.APIVersion, CommandID: sequenceID(7116), ClientID: clientID, ConversationID: &conversationID, Type: protocol.CommandQueueEdit, Payload: protocol.QueueEditPayload{MessageID: sequenceID(7112), Content: protocol.TextContent("edited")}}
	result, err = connection.Command(context.Background(), edit)
	require.NoError(t, err)
	requireCommandResult(t, result, protocol.CommandSucceeded, "")
	queueEvent = receiveLifecycle(t, connection.Events())
	require.Equal(t, "edited", queueEvent.Payload.(protocol.QueuePayload).Items[0].Content.Parts[0].Text)
	require.Equal(t, result, receiveLifecycle(t, connection.Events()))

	remove := protocol.Command{APIVersion: protocol.APIVersion, CommandID: sequenceID(7117), ClientID: clientID, ConversationID: &conversationID, Type: protocol.CommandQueueRemove, Payload: protocol.MessageReferencePayload{MessageID: sequenceID(7115)}}
	...
	queueEvent = receiveLifecycle(t, connection.Events())
	require.Equal(t, protocol.EventQueue, queueEvent.Type)
	require.Empty(t, queueEvent.Payload.(protocol.QueuePayload).Items)
...
func TestResyncAfterNewestEventEmitsSnapshotCheckpointThenStableResult(t *testing.T) {
	...
	conversationID := connection.ConversationID()
	command := protocol.Command{APIVersion: protocol.APIVersion, CommandID: sequenceID(7092), ClientID: clientID, ConversationID: &conversationID, Type: protocol.CommandResync, Payload: protocol.ResyncPayload{AfterEventID: checkpoint.EventID}}
...
func TestActiveAndQueueIdentifiersStayUniqueAndQueueLimitsRejectWithoutMutation(t *testing.T) {
	...
	for index := 0; index < protocol.MaxQueueItems; index++ {
	...
	require.NoError(t, err)
	requireCommandResult(t, result, protocol.CommandRejected, protocol.ErrorQueueFull)
	require.Equal(t, result, receiveLifecycle(t, connection.Events()))
}
...
func TestProviderEventsRequireTheActiveTurnAndUseProviderUserMessageAuthority(t *testing.T) {
	...
	require.Equal(t, protocol.EventError, malformed.Type)
	require.Equal(t, protocol.ErrorProviderMalformedStream, malformed.Payload.(protocol.ErrorPayload).Error.Code())
	...
	require.Equal(t, "question", user.Payload.(protocol.UserMessagePayload).Content.Parts[0].Text)
	session.events <- provider.NewCompletionEvent(turnID)
	require.Equal(t, protocol.EventCompletion, receiveLifecycle(t, connection.Events()).Type)
	require.Equal(t, protocol.LifecycleReady, receiveLifecycle(t, connection.Events()).Payload.(protocol.LifecyclePayload).State)
}
...
func TestSessionWideTerminalFailureInterruptsActiveTurnAndRecoversQueuedWork(t *testing.T) {
	...
	require.Equal(t, protocol.InterruptionProviderExit, interrupted.Payload.(protocol.InterruptionPayload).Reason)
	...
	require.Equal(t, protocol.EventQueue, receiveLifecycle(t, connection.Events()).Type)
	require.Equal(t, protocol.LifecycleConnecting, receiveLifecycle(t, connection.Events()).Payload.(protocol.LifecyclePayload).State)
	require.Equal(t, queuedTurnID, receiveLifecycle(t, session.submitted).TurnID)
	require.Equal(t, protocol.LifecycleResponding, receiveLifecycle(t, connection.Events()).Payload.(protocol.LifecyclePayload).State)
...
func TestTurnScopedTerminalFailureDoesNotPromoteToSessionCrash(t *testing.T) {
	...
	failure := receiveLifecycle(t, connection.Events())
	require.Equal(t, protocol.EventError, failure.Type)
	require.Equal(t, protocol.ErrorProviderProtocolFailure, failure.Payload.(protocol.ErrorPayload).Error.Code())
...
func TestNewRevisionAttachesToTheNextExistingQueuedMessage(t *testing.T) {
	...
	require.NoError(t, err)
	defer second.Close(context.Background())
	require.Equal(t, protocol.ContextPending, receiveLifecycle(t, second.Events()).Payload.(protocol.SnapshotPayload).ContextState)
	require.Equal(t, protocol.EventContext, receiveLifecycle(t, connection.Events()).Type)
	...
	require.NoError(t, err)
	drainEvents(t, connection.Events(), 2)
	require.Equal(t, protocol.EventQueue, receiveLifecycle(t, second.Events()).Type)
...
func TestNewerObservedRevisionInvalidatesQueuedContextBeforeDispatch(t *testing.T) {
	...
	require.Equal(t, protocol.EventContext, acceptedContext.Type)
	require.Equal(t, revisionC.Digest, acceptedContext.Payload.(protocol.ContextPayload).Digest)
	require.Equal(t, protocol.LifecycleResponding, receiveLifecycle(t, connection.Events()).Payload.(protocol.LifecyclePayload).State)
	state.mu.Lock()
...
func TestRemovingContextBearingHeadPausesQueueUntilReplacementIsSupplied(t *testing.T) {
	...
	require.NoError(t, err)
	drainEvents(t, connection.Events(), 2)
	remove := protocol.Command{APIVersion: protocol.APIVersion, CommandID: sequenceID(7221), ClientID: clientID, ConversationID: &conversationID, Type: protocol.CommandQueueRemove, Payload: protocol.MessageReferencePayload{MessageID: headMessageID}}
...
func replacementPage(previous protocol.Resource, markdown string, advance time.Duration) protocol.PageContext {
	resource := previous
...
func TestTurnDurableMutationsRetryOnlyFromProvenPreconditions(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{name: "prepare uncertain precondition", prepare: []repairMutation{{outcome: statepkg.CommitUncertain, err: failure}, {outcome: statepkg.CommitApplied, apply: true}}, promote: []repairMutation{{outcome: statepkg.CommitApplied, apply: true}}, wantStatus: protocol.CommandSucceeded, wantPrepare: 2, wantAccept: 1, wantPromote: 1},
		{name: "accept uncertain precondition", accept: []repairMutation{{outcome: statepkg.CommitUncertain, err: failure}, {outcome: statepkg.CommitApplied, apply: true}}, promote: []repairMutation{{outcome: statepkg.CommitApplied, apply: true}}, wantStatus: protocol.CommandSucceeded, wantPrepare: 1, wantAccept: 2, wantPromote: 1},
		{name: "promote uncertain precondition", promote: []repairMutation{{outcome: statepkg.CommitUncertain, err: failure}, {outcome: statepkg.CommitApplied, apply: true}}, wantStatus: protocol.CommandSucceeded, wantPrepare: 1, wantAccept: 1, wantPromote: 2},
		{name: "reconcile uncertain precondition", reconcile: []repairMutation{{outcome: statepkg.CommitUncertain, err: failure}, {outcome: statepkg.CommitApplied, apply: true}}, preflightErr: provider.NewProviderError(provider.ErrorContextTooLarge), wantStatus: protocol.CommandRejected, wantCode: protocol.ErrorContextTooLarge, wantPrepare: 1, wantReconcile: 2},
	} {
		t.Run(test.name, func(t *testing.T) {
...
func TestTurnDurableMutationDoesNotRetryAfterUnrelatedLoadedState(t *testing.T) {
	...
	state.preparations = []repairMutation{{outcome: statepkg.CommitUncertain, err: errors.New("uncertain"), mapping: &other}}
	state.mu.Unlock()
	conversationID := connection.ConversationID()
...
func TestAcceptanceUnknownRetainsPreparedStateAndDeduplicatesWithoutResubmit(t *testing.T) {
	...
	require.Equal(t, statepkg.CommitPrepared, state.mapping.Current.PreparedCommit.Phase)
...
func turnFixture(t *testing.T, base uint64) (*Broker, *repairState, *turnSession, *Connection, string, statepkg.Identity, protocol.Resource, protocol.PageContext) {
	t.Helper()
...
func submitCommand(commandID, clientID, conversationID, turnID, messageID, message string, page *protocol.PageContext) protocol.Command {
	return protocol.Command{APIVersion: protocol.APIVersion, CommandID: commandID, ClientID: clientID, ConversationID: &conversationID, Type: protocol.CommandSubmit, Payload: protocol.SubmitPayload{TurnID: turnID, MessageID: messageID, Content: protocol.TextContent(message), Context: page}}
}

func requireCommandResult(t *testing.T, event protocol.Event, status protocol.CommandStatus, code protocol.BrowserErrorCode) {
	t.Helper()
	require.Equal(t, protocol.EventCommandResult, event.Type)
	payload := event.Payload.(protocol.CommandResultPayload)
...
func drainEvents(t *testing.T, events <-chan protocol.Event, count int) {
	t.Helper()
