...
const (
	...
	MaxLaunchItems          = common.MaxProcessLaunchItems
	MaxLaunchAggregateBytes = common.MaxProcessLaunchAggregateBytes
	MaxImagesPerTurn        = agent.MaxImagesPerTurn
	MaxImageBytes           = agent.MaxImageBytes
	MaxTurnImageBytes       = agent.MaxTurnImageBytes
	MaxImageNameBytes       = agent.MaxImageNameBytes
)
...
type LaunchRequest = common.ProcessRequest

// Launcher starts one isolated provider process from an explicit specification.
type Launcher = common.ProcessLauncher
...
// the source to close.
type ManagedChild = common.ManagedProcess
...
func validImageMediaType(mediaType string) bool {
	return common.SupportedRasterMediaType(mediaType)
}
