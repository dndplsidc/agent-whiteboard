// Package attachment owns private, conversation-local image attachments
...
func (service *Service) Stage(ctx context.Context, request StageRequest) (Staged, error) {
	...
	if draftCount >= agent.MaxImagesPerTurn {
	...
	written, copyErr := io.Copy(io.MultiWriter(file, &content), io.LimitReader(request.Content, int64(agent.MaxImageBytes)+1))
	syncErr := file.Sync()
	closeErr := file.Close()
	...
	if written > int64(agent.MaxImageBytes) {
		_ = root.Remove(temporary)
	...
	if draftBytes+written > int64(agent.MaxTurnImageBytes) {
		_ = root.Remove(temporary)
		return Staged{}, ErrTurnLimit
	}
	if conversationBytes+written > agent.MaxConversationImageBytes {
		_ = root.Remove(temporary)
...
func (service *Service) Claim(ctx context.Context, request ClaimRequest) (Claimed, error) {
	...
	for position, reference := range request.Images {
		...
		descriptors[position] = protocol.ImageDescriptor{ImageID: record.ImageID, Name: reference.Name, MediaType: record.MediaType}
	}
...
func (service *Service) ImagesForMessage(ctx context.Context, conversationID, messageID string) ([]protocol.ImageDescriptor, error) {
	...
	result := make([]protocol.ImageDescriptor, 0)
...
func readSecureFile(root *os.Root, name string, expected int64) ([]byte, error) {
	...
	if err != nil || !secureRegular(info) || info.Size() != expected || expected <= 0 || expected > int64(agent.MaxImageBytes) {
		return nil, ErrStorage
	}
...
func validRecord(record record, conversationID string) bool {
	if common.ValidateID(record.ImageID) != nil || record.ConversationID != conversationID || common.ValidateID(record.ClientID) != nil || !record.Provider.Valid() || !record.Purpose.valid() || !validOrigin(record.Origin) || !common.SupportedRasterMediaType(record.MediaType) || record.Bytes <= 0 || record.Bytes > int64(agent.MaxImageBytes) || record.CreatedAt.IsZero() || record.Filename != record.ImageID+extensionFor(record.MediaType) {
		return false
	}
	switch record.State {
...
func descriptor(record record) protocol.ImageDescriptor {
	return protocol.ImageDescriptor{ImageID: record.ImageID, Name: record.Name, MediaType: record.MediaType}
}
...
func validName(value string) bool {
	return value != "" && utf8.ValidString(value) && len(value) <= agent.MaxImageNameBytes && !strings.ContainsRune(value, '\x00')
}
