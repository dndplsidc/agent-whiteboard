...
func (actor *conversation) commandInteractionRespond(results chan<- interactionWorkerResult, command agentprotocol.Command, payload agentprotocol.InteractionResponsePayload) agentprotocol.BrowserErrorCode {
	pending := actor.pendingInteractions[payload.RequestID]
	interactive, ok := actor.session.session.(provider.InteractiveSession)
	if pending == nil || pending.resolving || !ok || agentprotocol.InteractionKind(pending.request.Kind) != payload.Kind || !validInteractionResponseForRequest(pending.request, payload) {
...
func (actor *conversation) handleInteractionResult(attachments map[*attachment]struct{}, result interactionWorkerResult) {
	pending := actor.pendingInteractions[result.requestID]
	if pending == nil || !pending.resolving {
		if !result.automatic && result.commandID != "" {
			actor.completePendingCommand(attachments, result.commandID, result.clientID, agentprotocol.ErrorInvalidState)
		}
		return
	}
	if result.err != nil {
		...
		actor.publishShared(attachments, agentprotocol.InteractionResolvedPayload{
			RequestID: result.requestID,
			Kind:      agentprotocol.InteractionKind(pending.request.Kind),
	...
	actor.forgetInteraction(result.requestID)
	actor.publishShared(attachments, agentprotocol.InteractionResolvedPayload{RequestID: result.requestID, Kind: agentprotocol.InteractionKind(pending.request.Kind), OptionID: result.response.OptionID})
	if !result.automatic && result.commandID != "" {
...
func (actor *conversation) expirePendingInteractions(attachments map[*attachment]struct{}) {
	for requestID, pending := range actor.pendingInteractions {
		if pending.resolving {
			continue
		}
...
func validInteractionResponseForRequest(request provider.InteractionRequest, response agentprotocol.InteractionResponsePayload) bool {
	if request.ID != response.RequestID || agentprotocol.InteractionKind(request.Kind) != response.Kind || !validInteractionOption(request.Options, response.OptionID) {
		return false
	}
	switch request.Kind {
