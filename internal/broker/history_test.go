...
func TestCurrentHistoryPagesProviderMessagesWithStableBrowserIdentity(t *testing.T) {
	...
	command := historyCommand(sequenceID(9004), clientID, conversationID, agentprotocol.PageRequestPayload{Limit: 2})
...
func TestCurrentHistoryTimelineIsIsolatedFromOtherClientsLiveAndInReplay(t *testing.T) {
	...
	require.NoError(t, err)
	requireCommandResult(t, result, agentprotocol.CommandSucceeded, "")
	require.Equal(t, agentprotocol.EventTimeline, receiveLifecycle(t, first.Events()).Type)
	require.Equal(t, result, receiveLifecycle(t, first.Events()))

	resync := agentprotocol.Command{APIVersion: agentprotocol.APIVersion, CommandID: sequenceID(9010), ClientID: secondClientID, ConversationID: &conversationID, Type: agentprotocol.CommandResync, Payload: agentprotocol.ResyncPayload{AfterEventID: secondCheckpoint.EventID}}
	resyncResult, err := second.Command(context.Background(), resync)
	require.NoError(t, err)
	requireCommandResult(t, resyncResult, agentprotocol.CommandSucceeded, "")
	require.Equal(t, agentprotocol.EventSnapshot, receiveLifecycle(t, second.Events()).Type, "another client's targeted timeline must be absent from live delivery and replay")
...
func TestCurrentHistoryRedactsProviderFailureAndRejectsMalformedPage(t *testing.T) {
	...
	session.mu.Unlock()
	failed := historyCommand(sequenceID(9011), clientID, conversationID, agentprotocol.PageRequestPayload{})
	...
	require.NotContains(t, result.Payload.(agentprotocol.CommandResultPayload).Error.Message(), "/private")
	require.Equal(t, result, receiveLifecycle(t, connection.Events()))
	...
	session.mu.Unlock()
	malformed := historyCommand(sequenceID(9014), clientID, conversationID, agentprotocol.PageRequestPayload{})
...
func TestCurrentHistoryRejectsProviderPagesBeyondRequestedOrBrowserBoundsIdempotently(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			session.mu.Unlock()
			conversationID := connection.ConversationID()
			command := historyCommand(sequenceID(9041), clientID, conversationID, agentprotocol.PageRequestPayload{Limit: test.limit})
...
func TestHistoryWorkerDelaysQueuedDispatchWithoutBlockingProviderEvents(t *testing.T) {
	...
	session.mu.Unlock()
	history := historyCommand(sequenceID(9027), clientID, conversationID, agentprotocol.PageRequestPayload{})
	...
	require.Equal(t, agentprotocol.EventTimeline, receiveLifecycle(t, connection.Events()).Type)
...
func TestInterruptWaitsForHistoryWorkerWithoutOverlappingProviderCalls(t *testing.T) {
	...
	session.historyGate = gate
	session.mu.Unlock()
	history := historyCommand(sequenceID(9034), clientID, conversationID, agentprotocol.PageRequestPayload{})
	historyDone := make(chan commandResponse, 1)
	go func() {
		event, err := connection.Command(context.Background(), history)
		historyDone <- commandResponse{event: event, err: err}
	...
	receiveLifecycle(t, entered)
	interrupt := agentprotocol.Command{APIVersion: agentprotocol.APIVersion, CommandID: sequenceID(9035), ClientID: clientID, ConversationID: &conversationID, Type: agentprotocol.CommandInterrupt, Payload: agentprotocol.TurnReferencePayload{TurnID: turnID}}
	...
	requireCommandResult(t, historyResponse.event, agentprotocol.CommandSucceeded, "")
	...
	requireCommandResult(t, interruptResponse.event, agentprotocol.CommandSucceeded, "")
	require.EqualValues(t, 1, session.maximumProviderCalls.Load(), "history and interrupt must use one provider worker lane")
}

func historyCommand(commandID, clientID, conversationID string, payload agentprotocol.PageRequestPayload) agentprotocol.Command {
	return agentprotocol.Command{APIVersion: agentprotocol.APIVersion, CommandID: commandID, ClientID: clientID, ConversationID: &conversationID, Type: agentprotocol.CommandHistoryPage, Payload: payload}
}
