...
func TestIdleActorStopsAndLaterConnectResumesTheSameDurableSession(t *testing.T) {
	...
	requireBrokerCode(t, err, agentprotocol.ErrorReplayWindowUnavailable)
...
func TestCompletedActorWatcherCannotDeleteReplacementSlot(t *testing.T) {
	identity := agentstate.Identity{Origin: "https://example.com", Kind: agentstate.ResourceMarkdown, CapabilityID: sequenceID(10025), Provider: provider.NamePi}
	...
	broker := &Broker{registry: map[agentstate.Identity]*conversationSlot{identity: newSlot}}
...
func resyncCommand(commandID, clientID, conversationID, after string) agentprotocol.Command {
	return agentprotocol.Command{APIVersion: agentprotocol.APIVersion, CommandID: commandID, ClientID: clientID, ConversationID: &conversationID, Type: agentprotocol.CommandResync, Payload: agentprotocol.ResyncPayload{AfterEventID: after}}
}
