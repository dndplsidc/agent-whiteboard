...
func requireRecoveryCycle(t *testing.T, events <-chan agentprotocol.Event) {
	t.Helper()
	...
	require.Equal(t, agentprotocol.EventLifecycle, unavailable.Type)
	require.Equal(t, agentprotocol.LifecycleUnavailable, unavailable.Payload.(agentprotocol.LifecyclePayload).State)
	ready := receiveLifecycle(t, events)
	require.Equal(t, agentprotocol.EventLifecycle, ready.Type)
	require.Equal(t, agentprotocol.LifecycleReady, ready.Payload.(agentprotocol.LifecyclePayload).State)
}
...
func TestRecoveryStopsMalformedCandidateAndFailsClosed(t *testing.T) {
	...
	require.Equal(t, agentprotocol.ErrorProviderRecoveryFailed, receiveLifecycle(t, connection.Events()).Payload.(agentprotocol.ErrorPayload).Error.Code())
	require.Equal(t, agentprotocol.LifecycleUnavailable, receiveLifecycle(t, connection.Events()).Payload.(agentprotocol.LifecyclePayload).State)
...
func TestRecoveryNeverReplaysActiveTurnAndPreservesQueuedFollowUpOnFailure(t *testing.T) {
	...
	state := &repairState{mapping: &mapping, promotions: []repairMutation{{outcome: agentstate.CommitApplied, apply: true}}}
	...
	require.Equal(t, agentprotocol.ErrorProviderRecoveryFailed, recoveryFailure.Payload.(agentprotocol.ErrorPayload).Error.Code())
	require.Equal(t, agentprotocol.LifecycleUnavailable, receiveLifecycle(t, connection.Events()).Payload.(agentprotocol.LifecyclePayload).State)
...
func TestRecoveryReconcilesPreparedWithoutSubmitAndFailsClosed(t *testing.T) {
	tests := []struct {
		...
		{name: "durable ambiguity", state: provider.TurnAccepted, mutation: &repairMutation{outcome: agentstate.CommitUncertain, err: errors.New("uncertain")}},
	}
	for index, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			observed := agentstate.Revision{Digest: page.Digest, Revision: agentstate.RevisionInitial, SourceUpdatedAt: resource.UpdatedAt}
			mapping.Current.Observed = &observed
			mutation := repairMutation{outcome: agentstate.CommitApplied, apply: true}
			...
			if test.recovered {
				require.Equal(t, agentprotocol.EventLifecycle, result.Type)
				require.Equal(t, agentprotocol.LifecycleReady, result.Payload.(agentprotocol.LifecyclePayload).State)
			} else {
				require.Equal(t, agentprotocol.ErrorProviderRecoveryFailed, result.Payload.(agentprotocol.ErrorPayload).Error.Code())
				require.Equal(t, agentprotocol.LifecycleUnavailable, receiveLifecycle(t, connection.Events()).Payload.(agentprotocol.LifecyclePayload).State)
...
func TestRecoveryCompletesBlockedHistoryBeforeOldShutdownSettles(t *testing.T) {
	...
	connection := connected.(*Connection)
	receiveLifecycle(t, connection.Events())
	command := historyCommand(sequenceID(1583), clientID, connection.ConversationID(), agentprotocol.PageRequestPayload{Limit: 1})
	...
	require.Equal(t, agentprotocol.ErrorProviderCrashed, receiveLifecycle(t, connection.Events()).Payload.(agentprotocol.ErrorPayload).Error.Code())
	result := receiveLifecycle(t, connection.Events())
	requireCommandResult(t, result, agentprotocol.CommandRejected, agentprotocol.ErrorProviderCrashed)
	...
	case duplicate := <-connection.Events():
		if duplicate.Type == agentprotocol.EventCommandResult && duplicate.Payload.(agentprotocol.CommandResultPayload).CommandID == command.CommandID {
...
func TestRecoveryCompletesBlockedInterruptBeforeOldShutdownSettles(t *testing.T) {
	...
	interrupt := agentprotocol.Command{APIVersion: agentprotocol.APIVersion, CommandID: sequenceID(1591), ClientID: clientID, ConversationID: &conversationID, Type: agentprotocol.CommandInterrupt, Payload: agentprotocol.TurnReferencePayload{TurnID: turnID}}
	...
	requireCommandResult(t, interruptResult, agentprotocol.CommandRejected, agentprotocol.ErrorTurnInterrupted)
...
func TestNewerRevisionAttachedDuringRecoveryErasesStaleQueuedContext(t *testing.T) {
	...
	observed := agentstate.Revision{Digest: page1.Digest, Revision: agentstate.RevisionInitial, SourceUpdatedAt: resource1.UpdatedAt}
	mapping.Current.Observed = &observed
	state := &repairState{mapping: &mapping, promotions: []repairMutation{{outcome: agentstate.CommitApplied, apply: true}}}
	old := newTurnSession(mapping.Current.NativeSession.Value())
	...
	page2.Revision = agentprotocol.ContextReplacement
	page2.Markdown = "# replacement two\n"
	page2.Digest = contextdigest.Calculate([]byte(page2.Markdown), []byte(page2.CreatorContext))
	...
	page3.Revision = agentprotocol.ContextReplacement
	page3.Markdown = "# replacement three\n"
	page3.Digest = contextdigest.Calculate([]byte(page3.Markdown), []byte(page3.CreatorContext))
	...
	snapshot := receiveLifecycle(t, third.Events()).Payload.(agentprotocol.SnapshotPayload)
	require.Equal(t, agentprotocol.LifecycleUnavailable, snapshot.Lifecycle)
	close(shutdownGate)
	require.Equal(t, agentprotocol.EventContext, receiveLifecycle(t, connection.Events()).Type)
	require.Equal(t, agentprotocol.LifecycleReady, receiveLifecycle(t, connection.Events()).Payload.(agentprotocol.LifecyclePayload).State)
	require.Equal(t, agentprotocol.EventContext, receiveLifecycle(t, third.Events()).Type)
	require.Equal(t, agentprotocol.LifecycleReady, receiveLifecycle(t, third.Events()).Payload.(agentprotocol.LifecyclePayload).State)
...
func TestRecoveryPromotesCommitAcceptedWithExactRetryClassification(t *testing.T) {
	...
	state := &repairState{mapping: &mapping, promotions: []repairMutation{
		{outcome: agentstate.CommitNotApplied, err: errors.New("not applied")},
