...
type fakeBackend struct {
	...
	commands    []agentprotocol.Command
}

func (b *fakeBackend) Connect(_ context.Context, _ string, command agentprotocol.Command) (Connection, error) {
	connection := &fakeConnection{events: make(chan agentprotocol.Event, 8), closed: make(chan struct{})}
...
func (c *fakeConnection) Command(_ context.Context, command agentprotocol.Command) (agentprotocol.Event, error) {
	...
	case c.events <- event:
	case <-c.closed:
		return agentprotocol.Event{}, context.Canceled
	}
	return event, nil
}

func connectCommand() agentprotocol.Command {
	...
	return agentprotocol.Command{
		...
		Payload: agentprotocol.ConnectPayload{
			...
			Resource:      agentprotocol.Resource{Kind: agentprotocol.ResourceMarkdown, ID: resourceID, CreatedAt: created, UpdatedAt: created},
			ContextDigest: strings.Repeat("a", 64),
		},
	}
}

func ordinaryCommand() agentprotocol.Command {
	...
	return agentprotocol.Command{
		...
		Type:           agentprotocol.CommandNew,
		Payload:        agentprotocol.EmptyPayload{},
	}
}

func snapshotEvent() agentprotocol.Event {
	return agentprotocol.Event{
		APIVersion:     agentprotocol.APIVersion,
		EventID:        eventID,
		ConversationID: conversation,
		Type:           agentprotocol.EventSnapshot,
		Timestamp:      time.Unix(2, 0).UTC(),
		Payload:        agentprotocol.SnapshotPayload{Lifecycle: agentprotocol.LifecycleReady, Queue: []agentprotocol.QueueItem{}, ContextState: agentprotocol.ContextAccepted},
	}
}

func resultEvent(id string) agentprotocol.Event {
	return agentprotocol.Event{
		APIVersion:     agentprotocol.APIVersion,
		...
		Payload:        agentprotocol.CommandResultPayload{CommandID: id, Status: agentprotocol.CommandSucceeded},
	}
}
...
func encodeCommand(t *testing.T, command agentprotocol.Command) []byte {
	t.Helper()
...
func TestCORSAndLegacyPNAExactMatrix(t *testing.T) {
	...
	request, err = http.NewRequest(http.MethodOptions, running.baseURL+agentprotocol.ConnectPath, nil)
	require.NoError(t, err)
	request.Header.Set("Origin", trustedOrigin)
...
func TestHeaderLimitAndExactHostOnRawListener(t *testing.T) {
	...
	_, err = fmt.Fprintf(connection, "GET %s HTTP/1.1\r\nHost: %s\r\nOrigin: %s\r\nX-Fill: %s\r\n\r\n", agentprotocol.StatusPath, running.server.Host(), trustedOrigin, strings.Repeat("x", 40<<10))
	require.NoError(t, err)
...
func TestWebSocketAndFallbackParityTrustRemovalAndShutdown(t *testing.T) {
	running := startServer(t)
	wsURL := url.URL{Scheme: "ws", Host: running.server.Host(), Path: agentprotocol.ConnectPath}
	dialer := websocket.Dialer{Subprotocols: []string{agentprotocol.WebSocketSubprotocol}, EnableCompression: false}
	...
	require.Equal(t, agentprotocol.WebSocketSubprotocol, socket.Subprotocol())
	require.NoError(t, socket.WriteMessage(websocket.TextMessage, encodeCommand(t, connectCommand())))
	_, eventBytes, err := socket.ReadMessage()
	require.NoError(t, err)
	wsEvent, err := agentprotocol.DecodeEvent(eventBytes)
	require.NoError(t, err)
	assert.Equal(t, agentprotocol.EventSnapshot, wsEvent.Type)
	require.NoError(t, socket.WriteMessage(websocket.TextMessage, encodeCommand(t, ordinaryCommand())))
	_, eventBytes, err = socket.ReadMessage()
	require.NoError(t, err)
	wsResult, err := agentprotocol.DecodeEvent(eventBytes)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	streamEvent, err := agentprotocol.DecodeEvent(bytes.TrimSpace(line))
	require.NoError(t, err)
	...
	_, eventBytes, err = socket.ReadMessage()
	require.NoError(t, err)
	preservedResult, err := agentprotocol.DecodeEvent(eventBytes)
	require.NoError(t, err)
	...
	commandEvent, err := agentprotocol.DecodeEvent(commandBytes)
	require.NoError(t, err)
	assert.Equal(t, wsResult.Type, commandEvent.Type)

	denied := running.request(t, http.MethodPost, agentprotocol.ConnectPath, trustedOrigin, encodeCommand(t, connectCommand()))
