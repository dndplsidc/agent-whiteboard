	...
	for _, turn := range turns {
		...
		for _, item := range turn.Items {
			...
			case "userMessage":
				...
				envelope, parseErr := contentturn.Parse([]byte(text))
			...
			case "agentMessage":
				...
				items = append(items, provider.HistoryItem{TurnID: user.TurnID, MessageID: contentturn.AssistantMessageID(user.TurnID), Role: provider.HistoryAssistant, Text: bounded(cleanText(item.Text), provider.MaxHistoryItemBytes), CreatedAt: at})
			}
		}
	}
